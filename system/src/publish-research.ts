...
export function publishResearchDocument(doc: Omit<ResearchDocument, "publication_date" | "status"> & {
  ...
  // The /research/[id] page links to /research/[id].pdf as the
  // download. That file is generated by generate-research-pdfs.ts
  // against a running dev server. The PDF generator can't run from
  // inside this helper (it needs Puppeteer + a live next dev), so
  // surface a loud reminder instead of silently shipping a doc with
  // a broken download.
  ...
  if (!fs.existsSync(pdfPath)) {
    console.warn(
      `\n[PUBLISH] WARNING — no PDF at public/research/${doc.registry_id}.pdf yet.\n` +
        `  The /research/${doc.registry_id} page links to a Download PDF that will 404 until you run:\n` +
        `    (in website/) npm run dev    # leave running\n` +
        `    (in system/)  npx tsx scripts/generate-research-pdfs.ts --id=${doc.registry_id}\n` +
        `  then commit the generated PDF before pushing.\n`,
    );
  }
}
