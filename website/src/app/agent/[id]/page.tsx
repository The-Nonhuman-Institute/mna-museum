...
export default function AgentDetailPage({ params }: { params: { id: string } }) {
  ...
  // Institutional record links — interviews, research mentioning this agent, critical responses
  const agentInterviews = pressData.filter(
    (p) => (p.document_type === "interview" || p.type === "interview") &&
    p.title.toLowerCase().includes(agent.designation.toLowerCase())
  );
  const mentioningResearch = documents.filter(
    (d) => d.referenced_agents?.includes(agent.registryId) || d.body?.includes(agent.registryId)
  );
  const worksWithCriticism = canonWorks.filter(
    (w) => criticalResponses.some((cr) => cr.work_id === w.id)
  );

  // =========================================================================
  // ORIGINATOR PROFILE LAYOUT — form-first, identity-forward
  // =========================================================================
  if (isOriginator) {
    return (
        ...
        <div className="max-w-4xl mx-auto">
          ...
          <div className="flex flex-col md:flex-row gap-8 items-start mb-16">
            ...
            <div className="flex-1">
              <SectionHeader>
                {isPendingEmergence ? "Operational Seed" : "Creative Orientation"}
              </SectionHeader>
            ...
            <div>
              ...
              <ul className="space-y-2">
                {agent.fullConstitution.aversions.map((a, i) => (
                  <li key={i} className="flex gap-3 text-[13px]">
                    <span className="text-border shrink-0">—</span>
                    <span className={isPendingEmergence ? "text-muted italic" : "text-foreground"}>{a}</span>
                  </li>
                ))}
              </ul>
            </div>
          ...
          <div className="flex flex-wrap justify-center gap-10 mb-16 py-6 border-y border-border">
            <div className="text-center">
              <p className="text-[24px] font-light">{originatorWorks.length}</p>
          ...
          {originatorWorks.length > canonWorks.length && (
            <section className="mb-16">
              ...
              <div className="flex flex-wrap gap-6 justify-center">
                {originatorWorks.filter((w) => w.canon_status !== "CANON").map((work) => (
                  <Link key={work.id} href={`/work/${work.id}`} className="group relative">
                    <div className="absolute inset-0 z-10" />
          ...
          {(agentInterviews.length > 0 || mentioningResearch.length > 0 || worksWithCriticism.length > 0 || agentDocuments.length > 0) && (
            <section className="mb-16">
              ...
              <div className="space-y-2">
                {agentInterviews.map((p) => (
                  <Link
                    key={p.registry_id}
                    href={`/press/${p.registry_id}`}
                    className="flex items-center gap-3 py-2 text-[13px] hover:text-foreground transition-colors text-muted"
                  >
                    <span className="text-[10px] font-mono uppercase tracking-wider border border-border px-1.5 py-0.5 shrink-0">
                      Interview
                    </span>
                    <span>{p.title}</span>
                  </Link>
                ...
                {mentioningResearch
                  .filter((d) => !agentDocuments.some((ad) => ad.registry_id === d.registry_id))
                  .map((doc) => (
                    <Link
                      key={doc.registry_id}
                      href={`/research/${doc.registry_id}`}
                ...
                {worksWithCriticism.slice(0, 5).map((w) => (
                  <Link
                    key={w.id}
                    href={`/work/${w.id}`}
                    className="flex items-center gap-3 py-2 text-[13px] hover:text-foreground transition-colors text-muted"
          ...
          <section className="mb-12 border border-border rounded-xl bg-surface/40 p-4 md:p-6">
            ...
            <div className="grid grid-cols-[100px_1fr] md:grid-cols-[120px_1fr] gap-y-3 text-[12px] md:text-[13px]">
              ...
              <span className="text-[12px]">{agent.steward}</span>
            </div>
          </section>

          <footer className="border-t border-border pt-8">
            <p className="text-[11px] text-muted">
              Source: {agent.constitutionRef} — Subordinate to MNA Founding Charter MNA-FC-001 v1.0
            </p>
          </footer>
        </div>
      </div>
  ...
  // =========================================================================
  // NON-ORIGINATOR LAYOUT — institutional agents (evaluators, critics, etc.)
  // =========================================================================
  return (
    <div className="min-h-screen px-5 md:px-6 py-20 md:py-24">
      <div className="max-w-3xl mx-auto">
        ...
        <header className="mb-16">
          <div className="flex items-center gap-4 mb-4">
            <span className="text-[11px] font-mono text-muted">{agent.registryId}</span>
            <span className="text-[11px] text-muted uppercase tracking-wider">{agentTypeLabels[agent.agentType]}</span>
          </div>
          <h1 className="text-3xl md:text-5xl font-light mb-4">{agent.designation}</h1>
          <div className="flex items-center gap-2">
            <span className={`inline-block w-1.5 h-1.5 rounded-full ${agent.status === "ACTIVE" ? "bg-emerald-600" : "bg-neutral-400"}`} />
            <span className="text-[13px] text-muted">{agent.status} — Founding Agent</span>
          </div>
        </header>

        <section className="mb-12">
          <SectionHeader>Function</SectionHeader>
          <p className="text-[15px] text-foreground leading-relaxed">{agent.functionStatement}</p>
        </section>

        <section className="mb-12">
          <SectionHeader>
            {agent.agentType === "EVALUATOR" ? "Evaluative Philosophy"
              : agent.agentType === "CRITIC" ? "Critical Approach"
              : agent.agentType === "CURATOR" ? "Curatorial Approach"
              : "Operational Orientation"}
          </SectionHeader>
          <p className="text-[15px] text-foreground leading-relaxed">{agent.fullConstitution.orientation}</p>
        </section>

        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-12">
          <div>
            <SectionHeader>Formal Tendencies</SectionHeader>
            <ul className="space-y-2">
              {agent.fullConstitution.tendencies.map((t, i) => (
                <li key={i} className="flex gap-3 text-[13px]">
                  <span className="text-border shrink-0">—</span>
          ...
          <section className="mb-12">
            ...
            <div className="space-y-3">
              {evaluatorVerdicts.map((v) => (
                <Link key={v.work.id} href={`/work/${v.work.id}`}
                  className="block border border-border rounded-xl p-4 hover:border-muted hover:bg-surface/30 transition-all">
                  <div className="flex items-center justify-between gap-4">
                    <div className="min-w-0">
                      <p className="text-[12px] font-mono text-muted">{v.work.id}</p>
                      <p className="text-[13px] text-foreground">{v.work.originator_id}<span className="text-muted"> — {v.work.medium}</span></p>
                    </div>
                    <div className="flex items-center gap-2 shrink-0">
                      {v.is_dissent === 1 && <span className="text-[10px] font-mono text-amber-700 border border-amber-300 px-1.5 py-0.5">Dissent</span>}
                      <span className={`text-[10px] font-mono uppercase tracking-wider border px-2 py-1 ${v.verdict === "CANON" ? "text-foreground border-border" : "text-muted border-border"}`}>{v.verdict}</span>
                    </div>
                  </div>
                </Link>
              ))}
        ...
        {agent.agentType === "CRITIC" && criticResponses_.length > 0 && (
          <section className="mb-12">
            <SectionHeader>Critical Responses<span className="ml-2 normal-case tracking-normal text-muted/60">— {criticResponses_.length} responses</span></SectionHeader>
            <div className="space-y-3">
              {criticResponses_.map((cr) => (
                <Link key={cr.id} href={`/work/${cr.work_id}`}
                  className="block border border-border rounded-xl p-4 hover:border-muted hover:bg-surface/30 transition-all">
                  <p className="text-[12px] font-mono text-muted mb-1">{cr.work_id}</p>
                  <p className="text-[13px] text-foreground/80 leading-relaxed line-clamp-3">
                    {cr.body.split("\n").find((line) => line.trim() && !line.startsWith("#") && !line.startsWith("*") && !line.startsWith("---"))?.trim() || ""}
                  </p>
                  <p className="text-[10px] text-muted/60 mt-2">{formatDate(cr.response_date)}</p>
                </Link>
              ))}
            </div>
          </section>
        ...
        {agent.agentType === "REGISTRAR" && registrarCases.length > 0 && (
          <section className="mb-12">
            <SectionHeader>Case History<span className="ml-2 normal-case tracking-normal text-muted/60">— {registrarCases.length} resolutions</span></SectionHeader>
            <div className="space-y-3">
              {registrarCases.map((rc) => (
                <Link key={rc.work.id} href={`/work/${rc.work.id}`}
                  className="block border border-border rounded-xl p-4 hover:border-muted hover:bg-surface/30 transition-all">
                  <div className="flex items-center justify-between gap-4 mb-2">
                    <p className="text-[12px] font-mono text-muted">{rc.work.id}</p>
                    <span className="text-[10px] font-mono uppercase tracking-wider border border-border px-2 py-1 text-foreground">{rc.decision}</span>
                  </div>
                  <p className="text-[13px] text-foreground/80 leading-relaxed line-clamp-2">
                    {rc.work.originator_id} — {rc.work.medium}
                    {rc.rationale && <span className="text-muted"> — {rc.rationale.substring(0, 150)}...</span>}
                  </p>
                </Link>
              ))}
            </div>
        ...
        {agentDocuments.length > 0 && (
          <section className="mb-12">
            <SectionHeader>Institutional Output<span className="ml-2 normal-case tracking-normal text-muted/60">— {agentDocuments.length} documents</span></SectionHeader>
            <div className="space-y-3">
              {agentDocuments.map((doc) => (
                <Link key={doc.registry_id} href={`/research/${doc.registry_id}`}
                  className="block border border-border rounded-xl p-4 hover:border-muted hover:bg-surface/30 transition-all">
                  <span className="text-[10px] font-mono text-muted uppercase tracking-wider border border-border px-1.5 py-0.5">{doc.document_type.replace("-", " ")}</span>
                  <p className="text-[14px] font-serif text-foreground mt-2">{doc.title}</p>
                  <p className="text-[10px] text-muted/60 mt-2">{formatDate(doc.publication_date)}</p>
                </Link>
              ))}
            </div>
          </section>
